...
export default function MessageInput() {
    ...
    const { mutate: sendMessageMutate, isPending: isSendingMessage } = useSendMessage(); // Use the mutation hook
    ...
    const handleSendMessage = () => {
        ...
        if (trimmedMessage === '' || !selectedChat || !currentUser || isSendingMessage) return; // Prevent sending if pending
        ...
        console.log(`Calling mutation to send message to ${selectedChat.type} chat ${selectedChat.id}:`, trimmedMessage);
    ...
    return (
        <div className={`flex items-center p-4 border-t border-border ${!selectedChat || !socket?.connected ? 'opacity-50 pointer-events-none' : ''}`}>
            ...
            <TextareaAutosize
                ...
                placeholder={isDisabled ? (isSendingMessage ? "Sending..." : (socket?.connected ? "Select a chat" : "Connecting...")) : "Type a message..."}
