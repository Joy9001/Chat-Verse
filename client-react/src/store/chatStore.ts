...
interface ChatState {
    ...
    updatePrivateChatBlockStatus: (userId: string, isBlocked: boolean, blockedByMe: boolean) => void;
    // New actions
...
export const useChatStore = create<ChatState>((set, get) => ({
        ...
        if (chat && chat.type === 'private') {
            set((state) => ({
                privateChats: state.privateChats.map(pc =>
                    pc.id === chat.id
                        ? { ...pc, isBlocked: chat.isBlocked, blockedByMe: chat.blockedByMe, amIBlocked: chat.amIBlocked }
                        : pc
                )
            }));
        }
        set({ selectedChat: chat, messages: [], isLoadingMessages: false, chatError: null });
    },
    setMessages: (messages) => set({ messages: messages, isLoadingMessages: false }),
    ...
    updatePrivateChatBlockStatus: (userId, isBlocked, blockedByMe) => set((state) => {
        ...
        const newSelectedChat = state.selectedChat?.id === userId && state.selectedChat.type === 'private'
            ? { ...state.selectedChat, isBlocked, blockedByMe, amIBlocked: isBlocked && !blockedByMe } as PrivateChat
            : state.selectedChat;
