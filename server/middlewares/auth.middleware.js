...
export const isAuthenticated = (req, res, next) => {
	...
	passport.authenticate('jwt', { session: false }, async (err, user, info) => {
		...
		if (info?.message === 'No auth token' || info?.name === 'JsonWebTokenError' || info?.name === 'TokenExpiredError') {
			try {
				...
				if (Date.now() - refreshtokendb.updatedAt.getTime() > 1000 * 60 * 60 * 24) {
					const newRefreshToken = jwt.sign({ user: decoded.user }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: '7d' });
					refreshtokendb.refreshToken = newRefreshToken;
					await refreshtokendb.save();
					res.cookie('refreshToken', newRefreshToken, { httpOnly: true, secure: true, sameSite: 'strict', maxAge: 1000 * 60 * 60 * 24 * 7 });
				}

				const accessToken = jwt.sign({ user: decoded.user }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '30m' });
				res.cookie('accessToken', accessToken, { httpOnly: true, secure: true, sameSite: 'strict', maxAge: 1000 * 60 * 30 });

				const fullUser = await User.findById(decoded.user).select('-password');
				if (!fullUser) {
					console.error('User not found in DB after successful refresh token validation for ID:', decoded.user);
					res.clearCookie('accessToken');
					res.clearCookie('refreshToken');
				...
				if (error instanceof jwt.JsonWebTokenError || error instanceof jwt.TokenExpiredError) {
					...
					console.error('Unexpected error during refresh token processing:', error);
