  ...
  leftPeople.forEach(async (person) => {
    ...
    if (users && users.includes(personUsername)) {
      ...
      if (person.classList.contains("active")) {
        ...
        chat_head_status.classList.contains("online")
          ? null
          : chat_head_status.classList.add("online");
      }
    } else {
      status.classList.contains("online")
...
socket.on("newMessage", async (message, senderUsername, callback) => {
  ...
  if (sender === "") {
    ...
    popupPeople.forEach((person) => {
      ...
      if (personUsername === senderUsername) {
        let personName = person.querySelector(
          ".new-chat-people-name",
        ).innerText;
...
socket.on("deleteMessage", (dltMsgId, senderUsername) => {
  ...
  if (activePerson) {
    let personUnsername =
      activePerson.querySelector(".chat-username").innerText;
    ...
    try {
      ...
      popupPeople.forEach((person) => {
        ...
        if (personUsername === oldUserDetails.username) {
          person.querySelector(".new-chat-people-name").textContent =
            newUserDetails.name;
          person.querySelector(".new-chat-people-username").textContent =
            newUserDetails.username;
          person.querySelector(".new-chat-people-avatar").src =
      ...
      allLeftSideUser.forEach((user) => {
        ...
        if (username === oldUserDetails.username) {
          ...
          if (user.classList.contains("active")) {
            // chat head
            let chat_head = document.querySelector(".chats-head");
            chat_head.querySelector(".chat-img img").src =
              newUserDetails.avatar;
            chat_head.querySelector("#chat-head-name").textContent =
              newUserDetails.name;

            // chat head popup
            document.querySelector(".to-user-info-popup-details img").src =
              newUserDetails.avatar;
            ...
            toUserProfileSec.querySelector(".to-user-profile-sec-img img").src =
              newUserDetails.avatar;
...
socket.on("join-group", (data) => {
  ...
  if (currentUserName !== data.creatorName)
    createLeftsideGroup(data.groupInfo, false);
});
...
socket.on("group-message", (data, callback) => {
  ...
  if (findGroup.classList.contains("active")) {
    ...
    let toUserMsg = toUserGroupMsgComponent(
      message,
      msgId,
      msgTime,
      senderName,
    ...
    if (unreadMsgCount === 1) {
      ...
      showNotification(
        `You have ${unreadMsgCount} new messages in "${groupName}" group`,
      );
    }
