...
const addChatPopupInitialState =
  document.querySelector("#add-chat-popup").innerHTML;
...
document.querySelector("#transparent-modal").addEventListener("click", () => {
  ...
  showGroupMembersBtn.parentElement.open
    ? (showGroupMembersBtn.parentElement.open = false)
    : null;
...
document
  ...
  .addEventListener("click", () => {
    ...
    const gender = DOMPurify.sanitize(
      document.querySelector("#change-details-gender option:checked").value,
    );
    ...
    fetch("/api/change-details", {
      ...
      .catch((err) => {
        ...
        ChatFunctions.showNotification(
          "Error in changing details! Please refresh the page and try again.",
        );
      });
  });
...
document
  .querySelector(".all-group-members")
  .addEventListener("click", (event) => {
    // console.log('event.target', event.target)
...
document
  .querySelector("#leave-and-delete-group")
  .addEventListener("click", () => {
    let groupId = document.querySelector(".group-child.active").dataset.id;
    ...
    fetch("/group-chat-api/leave-and-delete-group", {
      ...
      .then((data) => {
        if (data.success) {
          let groupChild = document.querySelector(
            `.group-child[data-id="${groupId}"]`,
          );
          groupChild.remove();
...
document
  .querySelector("#all-group-members-btn")
  .addEventListener("click", () => {
    let dropdown = document.querySelector(
      "#all-group-members-btn",
    ).parentElement;
    if (dropdown.open) {
      return;
    }
    ...
    fetch("/group-chat-api/get-group-members", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          let noOfMembersDiv =
            groupMembersParent.previousElementSibling.querySelector("span");
          ...
          alertText.textContent =
            "Error in getting group members. Try refreshing the page.";
          alert.classList.remove("hidden");
          setTimeout(() => {
            alert.classList.add("hidden");
            alertText.textContent = "";
...
const initializeAddChatPopupEventListeners = () => {
  ...
  document.querySelector("#new-chat-tab").addEventListener("click", () => {
    ...
    let newGroupTabContent = document.querySelector(
      ".new-group-tab-content-parent",
    );

    newChatTab.classList.contains("tab-active")
      ? null
      : newChatTab.classList.add("tab-active");
    newGroupTab.classList.contains("tab-active")
      ? newGroupTab.classList.remove("tab-active")
      : null;

    newChatTabContent.classList.contains("hidden")
      ? newChatTabContent.classList.remove("hidden")
      : null;
    newGroupTabContent.classList.contains("hidden")
      ? null
      : newGroupTabContent.classList.add("hidden");
  });
  ...
  document.querySelector("#new-group-tab").addEventListener("click", () => {
    ...
    newChatTab.classList.contains("tab-active")
      ? newChatTab.classList.remove("tab-active")
      : null;
    newGroupTab.classList.contains("tab-active")
      ? null
      : newGroupTab.classList.add("tab-active");

    newChatTabContent.classList.contains("hidden")
      ? null
      : newChatTabContent.classList.add("hidden");
    newGroupTabContent.classList.contains("hidden")
      ? newGroupTabContent.classList.remove("hidden")
      : null;
  });

  // Search people for chat
  document
    .getElementById("new-chat-search")
      ...
      try {
        ...
        if (queryText === "") {
          popupPeople.forEach((person) => {
            person.classList.contains("hidden")
  ...
  document
    .querySelector(".all-new-chat-people")
    .addEventListener("click", (event) => {
  ...
  document
    .querySelector("#new-group-done-btn")
    .addEventListener("click", () => {
      document
        .querySelector(".create-group-tab-content")
  ...
  document
    .getElementById("new-group-search")
    .addEventListener("keyup", async (event) => {
      event.preventDefault();
      try {
        const queryText = event.target.value.toLowerCase();
  ...
  document
    .querySelector(".all-new-group-people")
    .addEventListener("click", (event) => {
      ...
      let checkBox = null;
      let selectedPeopleParent = document.querySelector(
        ".new-group-selected-people-parent",
      );
      let selectedPeople = document.querySelector(".new-group-selected-people");
      ...
      if (checkBox && checkBox.checked) {
        selectedPeopleParent.classList.contains("hidden")
          ? selectedPeopleParent.classList.remove("hidden")
          : null;
        document
          .querySelector(".all-new-group-people")
          .classList.replace("h-64", "h-46");
        ...
        for (let person of selectedAllPeople) {
          ...
          if (
            person.querySelector("img").alt ===
            selectedPerson.querySelector("img").alt
          ) {
            person.remove();
          }
        }
        ...
        if (selectedPeople.children.length === 0) {
          selectedPeopleParent.classList.contains("hidden")
            ? null
            : selectedPeopleParent.classList.add("hidden");
          document
            .querySelector(".all-new-group-people")
            .classList.replace("h-46", "h-64");
        }
      }
    });
  ...
  document
    .querySelector("#change-group-avatar-btn")
    .addEventListener("click", () => {
  ...
  // Create group button
  document
    .querySelector("#group-create-btn")
    .addEventListener("click", async () => {
      // collect the data
      let groupMembersIds = Array.from(
        document.querySelector(".new-group-selected-people").children,
      ).map((member) => member.dataset.id);
      let groupName = DOMPurify.sanitize(
        document.querySelector("#group-name").value,
      );
      let groupDescription = DOMPurify.sanitize(
      ...
      await fetch("/group-chat-api/create-group", {
        ...
        .then((data) => {
          ...
          if (data.success) {
            ...
            ChatFunctions.showNotification(
              "Error in creating group. Please try again!",
            );
          }
        })
        .catch((err) => {
