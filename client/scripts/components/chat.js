...
export const chatClicked = async (htmlElement, ...args) => {
  ...
  const clickedUser_Username =
    htmlElement.querySelector(".chat-username").innerText;
  let clickedUser = {};
  ...
  statusElement.classList.contains("online")
    ? (isOnline = true)
    : (isOnline = false);
...
export const handleChatHeadAndEnd = (clickedUser, isOnline = false) => {
  ...
  chat_head.classList.contains("hidden")
    ? chat_head.classList.remove("hidden")
    : null;
  chat_mid.classList.contains("hidden")
    ? chat_mid.classList.remove("hidden")
    : null;
  chat_end.classList.contains("hidden")
  ...
  to_user_info_popup.querySelector("#to-user-info-popup-name").textContent =
    clickedUser.name;
  to_user_info_popup.querySelector("#to-user-info-popup-username").textContent =
    clickedUser.username;
  to_user_info_popup.querySelector("#to-user-info-popup-img").src =
    clickedUser.avatar
      ? clickedUser.avatar
      : `https://avatar.iran.liara.run/username?username=${clickedUser.name.replace(/ /g, "+")}`;
  ...
  let groupInfoPopupOptions = document.querySelector(
    ".group-info-popup-options",
  );
  groupInfoPopupOptions.classList.contains("hidden")
    ? null
    : groupInfoPopupOptions.classList.add("hidden");
...
export const handleChats = (clickedUser) => {
  ...
  let toUserProfileSecImg = document.querySelector(
  ...
  let toUserProfileSecName = document.querySelector(
    ".to-user-profile-sec-name h1",
  );
  let toUserProfileSecUsername = document.querySelector(
  ...
  if (data.messages.length === 0) {
    ...
    data.messages.forEach((msg) => {
      ...
      if (msg.senderId === currentUserId) {
        let fromUserMsg = fromUserMsgComponent(
          msg.message,
          msg._id,
          utcToLocal(msg.createdAt).slice(0, 5),
        );
        msgContainerDiv.appendChild(fromUserMsg);
      } else {
        let toUserMsg = toUserMsgComponent(
          msg.message,
          msg._id,
          utcToLocal(msg.createdAt).slice(0, 5),
        );
...
export const addPeopleToChat = async (element) => {
  ...
  // Easter egg
  let currentUserName = document.querySelector(
  ...
  fetch("/chat/delete-message", {
    ...
    .then((data) => {
      ...
      if (data.message === "Message deleted") {
        ...
        days.forEach((day) => {
          if (
            day.nextElementSibling === null ||
            day.nextElementSibling.classList.contains("day")
          ) {
            day.remove();
          }
        });
